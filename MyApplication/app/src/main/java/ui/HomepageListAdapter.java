        ...
        for(int i = 0; i < temp.size(); ++i){
            if(temp.get(i).getAssignmentName().equals(recentDues.get(position).getAssignmentName())){
                if(temp.get(i).getCanReachNextLevel())
                    dueProjection.setText("You need to get at least " + String.format("%.2f",temp.get(i).getPercentToImprove()*100.0) + "% to get " + temp.get(i).getNextLevel());
                else
                dueProjection.setText("You need to get at least " + String.format("%.2f",temp.get(i).getPercentToImprove()*100.0) + "% to maintain " + temp.get(i).getNextLevel());
                break;
            }
        }
